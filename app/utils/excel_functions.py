...
def write_column(data: List[Any], start_row: int, column: str, ws: Worksheet, number_format: Optional[str] = None) -> None:
...
def write_report(L: ASFT_Data, R: ASFT_Data, estado: str, pavimento: str, output_folder: str='output'):

    def merge_thirds_column(data: ASFT_Data, col: str, ws: Worksheet, start_row = 11) -> pd.Series:
        length: int = len(data)
        third: float = length / 3
        row_A = start_row
        row_B = round(third) + start_row
        row_C = round(2 * third) + start_row
    ...
    def merge_average_row(data: ASFT_Data, col: str, ws: Worksheet, start_row: int, merge_range: int = 10):
        for row in range(start_row, len(data) + start_row, merge_range):
            ws.merge_cells(f'{col}{row}:{col}{row + merge_range - 1}')

    def format_cells(ws: Worksheet):
        border = Border(
    ...
    merge_average_row(L, 'D', ws, 11)
    merge_average_row(L, 'H', ws, 11)

    merge_thirds_column(L, 'E', ws, 11)
    merge_thirds_column(R, 'I', ws, 11)
